...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	for state in range(nS):
		...
		for action in range(nA):
			probability, nextstate, reward, terminal = P[state][action][0]
			possible_rewards.append(reward + gamma * probability * value_from_policy[nextstate])
		new_policy[state] = np.argmax(possible_rewards)
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
	...
	while True:
		...
		policy_change = (improved_policy != policy).sum()
		print("Policy changes: ", policy_change)
